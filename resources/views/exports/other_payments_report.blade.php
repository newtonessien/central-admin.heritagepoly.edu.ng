...
<head>
    <meta charset="UTF-8">
    <title>Other Payments Report</title>
    <style>
        @page { margin: 80px 30px 80px 30px; }
        body {
...
<header>
    ...
    <p>
        @if(!empty($filters['faculty_id']) && isset($reports[0]['faculty']) && $reports[0]['faculty'])
            Faculty of {{ $reports[0]['faculty'] }}
        @endif
 @if(!empty($filters['service_id']) && isset($reports[0]['service']) && $reports[0]['service'])
            — Service: {{ $reports[0]['service'] }}
        @endif
    {{-- return all program types if program_type_id  =  all --}}
        @if(!empty($filters['program_type_id']) && isset($reports[0]['program_type']) && $reports[0]['program_type'] )
            ({{ $reports[0]['program_type'] }})
        @endif
