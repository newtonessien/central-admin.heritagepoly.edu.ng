...
<div class="flex items-center gap-3 pt-2">
    <flux:button
        ...
        :disabled="!$program_type_id"   {{-- ✅ only program type is required now --}}
        size="sm"
        class="cursor-pointer"
    >
        <span wire:loading.remove wire:target="filterCandidates">
            Load {{ $status === 'approved' ? 'Approved' : ($status === 'pending' ? 'Pending' : 'All') }}
        </span>
    ...
    @if(!$program_type_id)
        <flux:text size="xs" class="text-zinc-500">
            Select a Program Type to load candidates (Faculty/Department/Program optional)
        </flux:text>
    @endif
